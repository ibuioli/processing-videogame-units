class Principal {
  ...
  int vel = 3;         //Velocidad del cuadro. Pueden probarse las diferecias al poner números más altos o más chicos. 
  boolean loop = true; //Bucle.
  ...
  void mover() {
    if (presDerecha == true || presIzquierda == true) {
      cuadro = animarSprites(numCuadros, cuadro, vel, loop);  //Llamamos a la función para animar los sprites
    }
